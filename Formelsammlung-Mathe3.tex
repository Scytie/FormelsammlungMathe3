...
$E(X) = n\frac{M}{N}$; $Var(X) = n \frac{M}{N} (1-\frac{M}{N}) \frac{N-n}{N-1}$\\
...
Tschebyscheff: $P(|X-E(X)| \geq \epsilon) \leq \frac{1}{\epsilon^2} V(X)$\\
...
$x_i \in [a,b]$ Stützstellen zu Interpolationspoly $p(x)$\\
$\forall x^* \exists \zeta \in [a,b]: |f(x^*) - p(x^*)| = \frac{f^{(n-1)} (\zeta)}{(n+1)!} \prod_{i=0}^{n}(x^* - x_i)$\\
...
Lösen GS: $y:=UX$ wie bei Cholesky\\
...
(1) $y=y_1 \cdot u(x)$ als Ansatz; (2)$z(x) = u'(x)$\\
(3) $y_2 = y_1 \cdot u(x)$\\
...
$(x^n)'  = nx^{\left( {n - 1} \right)}$\\
$(e^{ax})'  = ae^{ax}$;
$(b^x)'  = b^x \ln \left( b \right)$\\
$(\ln x )' = \frac{1}{x}$\\
$(\sin x)' = \cos x$;
$(\cos x)' =  - \sin x$\\
...
$( {\frac{{f\left( x \right)}}{{g\left( x \right)}}} )' = \frac{{f'\left( x \right)g\left( x \right) - f\left( x \right)g'\left( x \right)}}{{g^2 \left( x \right)}}$\\
...
$\sum_{i=1}^n \alpha_i u_i = 0 \Rightarrow \alpha_1=..\alpha_n=0$\\
